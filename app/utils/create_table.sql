    ...
    create table extract_record (
        doc_id	    number (100) primary key, --研报ID
        hisotry_id	number (100) not null, -- 抽取记录内部 ID
        doc_type	number (3)   not null, -- 文档类型
        pdf_path	varchar2(50) not null, -- 研报保存路径
    ...
    create table extract_item (
        doc_id	   number (100) foreign key not null, -- 研报ID
        item_id	   number (5) not null, -- 文本字段id
        item_name  varchar2(100) default '', -- 文本字段名称
        word	   varchar2(10000) default '', -- 抽取的值
    ...
    create table extract_table (
        ...
        table_path varchar2(100) default '', -- 表格保存的路径
    )